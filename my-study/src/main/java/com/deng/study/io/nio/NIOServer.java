    ...
    public void testBlockMode() throws IOException {
        ...
        while (true){
            ...
            for (SocketChannel channel : socketChannelList) {
                ...
                // 阻塞方法，线程停止运行，也就是当前线程阻塞后，会阻塞另外的方法去执行任务
                channel.read(buffer);
//                buffer.flip();
                debugAll(buffer);
                buffer.clear();
    ...
    /**
     * 测试非阻塞模式：没有连接或者数据读取的时候，也一直在运行，所以这种场景在生产上没有什么用，只是用来测试
     * @throws IOException
     */
    @Test
...
//            log.debug("connecting....");
            // 阻塞模式，意味着线程停止运行
            SocketChannel socketChannel = serverSocketChannel.accept(); // 这里不会阻塞，会继续后续的运行
